...
void usart_gpio_config(uint32_t band_rate) {
    ...
    /* 配置TX为推挽输出 50MHZ */
    gpio_output_options_set(BSP_USART_TX_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_2MHZ, BSP_USART_TX_PIN);
    /* 配置RX为推挽输出 50MHZ */
    gpio_output_options_set(BSP_USART_RX_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_2MHZ, BSP_USART_RX_PIN);
...
int _write(int file, char *data, int len)
{
    ...
    int i = 0;
    while (i < len)        // 地址为空或者值为空跳出
    {
        usart_send_data(*(data+i));  // 发送单个字符
        i++;
    }
    return len;
}
...
//	}
    ...
    if (usart_interrupt_flag_get(BSP_USART, USART_INT_FLAG_IDLE) ==
        SET)                                                 // 检测到帧中断
    {
